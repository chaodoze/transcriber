// Based on dado3212's apple-podcast-transcript-downloader
// https://github.com/dado3212/apple-podcast-transcript-downloader
// Modified to output file path on success for integration with Python wrapper
...
void printHelp() {
  ...
  printf("Usage:\n");
  printf("  FetchTranscript <episodeId> [--output-dir <dir>] [--cache-bearer-token]\n\n");
  printf("Options:\n");
  printf("  --output-dir <dir>     Save transcript to specified directory (default: current dir)\n");
  printf("  --cache-bearer-token   Use cached Bearer token if valid for 30 days\n");
...
int main(int argc, const char * argv[]) {
  @autoreleasepool {
    ...
    if ([podcastId rangeOfCharacterFromSet:nonDigits].location != NSNotFound) {
      fprintf(stderr, "Error: episodeId must be a number.\n");
      return 1;
    }
    ...
    if (!bearer || bearer.length < 10 || ![bearer hasPrefix:@"ey"]) {
      fprintf(stderr, "Failed to obtain Bearer token.\n");
      return 1;
    }
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:transcriptRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
      ...
      if (json[@"errors"] != nil) {
        fprintf(stderr, "Failed to fetch transcript for episode %s: %s\n",
                [podcastId UTF8String],
                [[json[@"errors"] description] UTF8String]);
        exitCode = 1;
        dispatch_semaphore_signal(sema);
        return;
      }
      ...
      NSString *ttmlFilename = [attrs[@"ttmlToken"] componentsSeparatedByString:@"/"].lastObject;

      if (!ttmlURL || !ttmlFilename) {
        fprintf(stderr, "No transcript available for episode %s\n", [podcastId UTF8String]);
        exitCode = 1;
        dispatch_semaphore_signal(sema);
        return;
      }
      ...
      NSString *outputPath = [outputDir stringByAppendingPathComponent:ttmlFilename];

      NSData *ttmlData = [NSData dataWithContentsOfURL:[NSURL URLWithString:ttmlURL]];
      if (!ttmlData) {
        fprintf(stderr, "Failed to download transcript from %s\n", [ttmlURL UTF8String]);
        exitCode = 1;
        dispatch_semaphore_signal(sema);
        return;
      }

      if (![ttmlData writeToFile:outputPath atomically:YES]) {
        fprintf(stderr, "Failed to save transcript to %s\n", [outputPath UTF8String]);
        exitCode = 1;
        dispatch_semaphore_signal(sema);
        return;
      }

      // Output the path on success (for Python wrapper to capture)
      printf("%s\n", [outputPath UTF8String]);
